...
- [ ] **B8 — Integracja frontu** (meta-faza: spina ~9 podsystemów frontu z `/api/v1/` — rozbita na podfazy shippable):
  - [x] **B8a — Fundament + Auth:** klient `fetch` na `/api/v1/` (token Knox w nagłówku, env `VITE_API_URL`, CORS, backend lokalnie), `AuthContext` (login/register/logout/me), chronione trasy (Account/Onboarding). Token w localStorage, auth dodatkowy (guest-first). Zweryfikowane E2E na żywym backendzie (login/persystencja/logout/zły-login, czysta konsola w buildzie prod). Spec+plan w `docs/superpowers/`.
